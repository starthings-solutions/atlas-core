...
function matchesSelector(el, selector) {
  ...
  if (selector === "[data-atlas-question]") return el.getAttribute("data-atlas-question") !== null;
...
test("deriveAtlasQueueKey uses explicit queueKey first", () => {
  const input = node("input", { type: "radio", name: "plan" });

  assert.equal(deriveAtlasQueueKey(input, { queueKey: "deployment-plan" }), "deployment-plan");
});

test("deriveAtlasQueueKey allows explicit empty queueKey to suppress derivation", () => {
  const button = node("button");
  node("section", { "data-atlas-question": "deployment-plan" }, [button]);

  assert.equal(deriveAtlasQueueKey(button, { queueKey: "" }), "");
});

test("deriveAtlasQueueKey groups controls inside data-atlas-question", () => {
  const first = node("button");
  const second = node("button");
  node("section", { "data-atlas-question": "deployment-plan" }, [first, second]);

  assert.equal(deriveAtlasQueueKey(first), "question:deployment-plan");
  assert.equal(deriveAtlasQueueKey(second), "question:deployment-plan");
});

test("deriveAtlasQueueKey groups radio options by scoped group name", () => {
  ...
  assert.equal(deriveAtlasQueueKey(planA), "radio:form:deploy:plan");
  assert.equal(deriveAtlasQueueKey(planB), "radio:form:deploy:plan");
});

test("deriveAtlasQueueKey keeps same radio names independent across scopes", () => {
  ...
  assert.notEqual(deriveAtlasQueueKey(first), deriveAtlasQueueKey(second));
});

test("deriveAtlasQueueKey does not infer plain button grouping without question metadata", () => {
  const button = node("button");

  assert.equal(deriveAtlasQueueKey(button), "");
});

test("deriveAtlasQueueKey keys checkbox toggles per checkbox, not per group", () => {
...
test("deriveAtlasQueueKey does not collide checkbox default values", () => {
...
test("deriveAtlasQueueKey keys named selects as fields", () => {
  const select = node("select", { name: "region" });
  node("form", { id: "deploy" }, [select]);

  assert.equal(deriveAtlasQueueKey(select), "field:form:deploy:region");
});
