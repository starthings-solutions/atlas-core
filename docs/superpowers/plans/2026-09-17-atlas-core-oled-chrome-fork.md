# Atlas Core OLED Chrome Fork Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Entregar um fork compatível do Atlas Core no qual o motor original de artefatos, Mermaid e Excalidraw permanece intacto, enquanto o chrome adota OLED Sunset Calm e a conversa desktop passa a abrir fechada em um drawer sobreposto que nunca redimensiona o iframe.
...
**Spec:** `docs/superpowers/specs/2026-09-17-atlas-oled-chrome-fork-design.md`

## Global Constraints

- Não editar `src/artifact-sdk.js`, a conversão Mermaid/Excalidraw, o frame do whiteboard ou os contratos `atlas:*`.
- Não alterar nomes públicos, `ATLAS_CORE_*`, portas, storage da fila, storage mobile existente, pacote ou comando nesta fase.
...
- Preserve: `docs/superpowers/specs/2026-09-17-atlas-oled-chrome-fork-design.md`
- Preserve: `docs/superpowers/plans/2026-09-17-atlas-oled-chrome-fork.md`
...
- [ ] **Step 1: Configurar a identidade Git fornecida pelo usuário**
  ...
  ```bash
  cd /home/dev/@development/atlas-core
...
- [ ] **Step 2: Versionar a decisão aprovada antes de criar a worktree**
  ...
  ```bash
  git add .gitignore docs/superpowers/specs/2026-09-17-atlas-oled-chrome-fork-design.md docs/superpowers/plans/2026-09-17-atlas-oled-chrome-fork.md
...
- [ ] **Step 3: Criar a worktree isolada**
  ...
  ```bash
  git worktree add .worktrees/atlas-oled-chrome -b feat/atlas-oled-chrome
  ```

- [ ] **Step 4: Instalar exatamente o lockfile e registrar o baseline**

  ```bash
  cd /home/dev/@development/atlas-core/.worktrees/atlas-oled-chrome
  pnpm install --frozen-lockfile
  pnpm exec prettier --write \
    docs/superpowers/specs/2026-09-17-atlas-oled-chrome-fork-design.md \
    docs/superpowers/plans/2026-09-17-atlas-oled-chrome-fork.md
...
- [ ] **Step 5: Rodar a caracterização browser existente**
  ...
  ```bash
  ATLAS_CORE_BROWSER_E2E=1 node --test test/mobile-conversation-sheet.browser.test.js
  ```
...
- [ ] **Step 4: Implementar o controlador unificado mínimo**
  ...
  Fazer breakpoint, `markSessionEnded` e boot chamarem `applyConversationState`. Ao entrar no mobile, zerar `desktopDrawerOpen`; ao sair, zerar `sheetOpen` e remover somente `atlas-core:sheet-open:${key}`. Só a abertura mobile escreve storage.
...
- [ ] **Step 1: Escrever um teste browser RED dos estilos computados**
  ...
  test(
    ...
    async () => {
      ...
      const browser = createChromeDriver({ temp, session: `atlas-chrome-theme-${process.pid}` });
      try {
...
- [ ] **Step 2: Rodar RED**
  ...
  ```bash
  ATLAS_CORE_BROWSER_E2E=1 node --test test/chrome-theme.browser.test.js
  ```
...
- [ ] **Step 6: Fazer GREEN e verificar ausência de vazamento para o iframe**
  ...
  ```bash
  node --test test/server.test.js test/chrome-client-queue.test.js
  ATLAS_CORE_BROWSER_E2E=1 node --test test/chrome-theme.browser.test.js test/mobile-conversation-sheet.browser.test.js
...
- [ ] **Step 7: Executar validação estrutural de conteúdo**
  ...
  Rodar este script Node somente-leitura a partir de `/home/dev/@development/atlas-core`; ele compara as duas cópias integrais com `oled-base.css` e valida a folha viva:
...
- [ ] **Step 2: Rodar as regressões browser do motor preservado**
  ...
  ```bash
  ATLAS_CORE_BROWSER_E2E=1 node --test --test-concurrency=1 \
...
- [ ] **Step 9: Commit final de QA e status**
  ...
  ```bash
  git add design-qa.md docs/superpowers/specs/2026-09-17-atlas-oled-chrome-fork-design.md
...
- [ ] **Step 10: Preparar handoff sem publicar**
  ...
  - observação de que package/CLI continuam `atlas-core` e nenhum remoto/release foi criado.
