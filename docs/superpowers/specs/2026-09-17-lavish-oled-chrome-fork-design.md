# Lavish OLED Chrome Fork — Design Specification
...
**Nome de trabalho:** Lavish OLED; o nome do pacote e o comando continuam `lavish-axi` nesta fase.

## 1. Resultado esperado

Criar um fork compatível do Lavish que preserve integralmente o motor original de geração, renderização e revisão de artefatos, mas substitua o chrome visual por uma interface OLED própria e menos intrusiva.
...
2. O renderer e o motor de componentes do Lavish são uma zona protegida.
...
- mensagens `lavish:*`, `data-lavish-*` e `source: "lavish-axi"`;
...
- `LAVISH_AXI_*`, porta, diretório de estado, health handshake e chaves de `sessionStorage`;
- comando e pacote `lavish-axi` durante a primeira fase.
...
O drawer controla somente visibilidade e interação. Estado de domínio continua pertencendo às estruturas atuais do Lavish.
...
- suítes browser relevantes com `LAVISH_AXI_BROWSER_E2E=1`.
