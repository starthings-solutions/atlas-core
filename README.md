...
- **Conversation layout** - This tree derives from [`kunchenguid/lavish-axi`](https://github.com/kunchenguid/lavish-axi); its MIT license and third-party attributions remain. On desktop, a permanent 48px rail starts closed on every load, and its 360px conversation drawer expands left over the artifact. Opening or closing it never changes the artifact iframe's left edge, right edge, or width, and desktop drawer state is never stored. Mermaid and Excalidraw editing, annotations, export, share, queueing, and agent replies remain compatible.
