...
- **Conversation layout** - On desktop, the permanent 48px rail remains visible and its 360px conversation drawer starts closed on every load, expanding left over the artifact when opened. Opening or closing it never changes the artifact iframe's left edge, right edge, or width, and desktop drawer state is never stored. Mermaid and Excalidraw editing, annotations, export, share, queueing, and agent replies remain compatible.
- **Reviewing on a phone** - Below 860px wide, the artifact takes the whole screen above a **Conversation** dock, and the conversation opens as a bottom sheet over it: tap the dock, swipe it up, or press the chevron to raise it; tap the dimmed artifact, swipe the sheet down, press the chevron, or press Escape to lower it. The dock reports what matters while the sheet is down - how many prompts are queued, a reply that arrived while you were reading, or whether the agent is listening - and the sheet stays open across a reload of the review page. The sheet sizes itself to the visible viewport and respects safe-area insets; if the keyboard or attachments leave little room, conversation content yields or scrolls while the send actions remain pinned above the bottom edge. In landscape the sheet covers the top bar as well. Its existing persistence and interaction behavior remain unchanged.
...
| `atlas-core update`             | Show how to update the source checkout with `git pull --ff-only` and reinstall it with `npm run install:local`; no registry lookup or install is performed.                                                                                                                                                                                                   |
...
| `atlas-core design`             | Show agent-facing design guidance, including optional CDN snippets and the whiteboard (Mermaid) opt-in snippet.                                                                                                                                                                                                                                               |
...
| `atlas-core setup plugin`       | Register the local Atlas Core installation as an [Agent Plugin](https://agent-plugins.org) in VS Code, Cursor, and GitHub Copilot CLI; opt-in, idempotent, no marketplace involved. Reload each client afterward.                                                                                                                                             |
...
| `atlas-core update`      | `--check`                   | Print the source-update instructions without changing anything.                                                                                                                                                                                                                                                               |
...
| `atlas-core server`      | `--verbose`                 | Log session and watcher events to stderr; can also be enabled with `ATLAS_CORE_DEBUG=1`. Detached server output is appended to `~/.atlas-core/server.log` (or `ATLAS_CORE_STATE_DIR/server.log`) for startup and crash diagnostics.                                                                                           |
...
pnpm run build:skill    # Regenerate the installable atlas-core skill
...
Atlas Core is derived from [`kunchenguid/lavish-axi`](https://github.com/kunchenguid/lavish-axi). The upstream Lavish project remains credited through its MIT license, commit history, changelog, and third-party notices.
